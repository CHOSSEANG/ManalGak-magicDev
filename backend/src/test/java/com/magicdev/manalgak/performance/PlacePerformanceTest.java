...
class PlacePerformanceTest {
    ...
    void 장소_추천_캐싱_성능_테스트() {
        ...
        PlaceResponse response1 = placeService.getRecommendedPlaces(meetingUuid, candidateId, purpose, limit);
        assertThat(response1.isFromCache()).isFalse();

        PlaceResponse response2 = placeService.getRecommendedPlaces(meetingUuid, candidateId, purpose, limit);
    ...
    void 캐시_만료_후_재호출_테스트() throws InterruptedException {
        ...
        String cacheKey = "places:meeting:" + meetingUuid + ":candidate:" + candidateId + ":" + purpose + ":" + limit;
    ...
    void 다른_파라미터_호출시_새로운_캐시_테스트() {
        String meetingUuid = "test-meeting-uuid-3";
        Long candidateId = 3L;
        String purpose1 = "DINING";
        String purpose2 = "CAFE";
        int limit = 5;

        PlaceResponse response1 = placeService.getRecommendedPlaces(meetingUuid, candidateId, purpose1, limit);
        assertThat(response1.isFromCache()).isFalse();

        PlaceResponse response2 = placeService.getRecommendedPlaces(meetingUuid, candidateId, purpose1, limit);
        assertThat(response2.isFromCache()).isTrue();

        PlaceResponse response3 = placeService.getRecommendedPlaces(meetingUuid, candidateId, purpose2, limit);
